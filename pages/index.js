...
export default function Home() {
  ...
  const nfteditioncontract = useEditionDrop("0x85393eD5Be92370E43705836498DA1c82198EBE8")

  const mintNFT = async () => {
    try {
      setIsclaiming(true);
      document.getElementById("mintbtn").disabled = true;
      await nfteditioncontract?.claim(0, 1)
      setHasclaimed(true);
    }
    catch (error) {
      setHasclaimed(false);
  ...
  useEffect(() => {
    ...
    const checkbalance = async () => {
      const nfts = await nfteditioncontract?.getOwned(address);
      setHasclaimed(nfts.length > 0);
    };
    checkbalance();
  }, [address, nfteditioncontract]);
  ...
  if (!address) {
    return (
      ...
      <Typewriter
       ...
       onInit={(typewriter)=> {
              ...
              typewriter
                ...
                .typeString("Claim Dope NFTS")
                .pauseFor(1000)
                .deleteAll()
                .typeString("Gain access to website")
                .pauseFor(1000)
                .deleteAll()
                .typeString("All hail the supreme cattos")
                .start();
       }}
       />
    </div>
      ...
      <div className={styles.container}>
          <button onClick={connectmetamask} type="button" id='metabtn' className="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2">
  ...
  if (hasclaimed)
  {
    return (
      <div className='final'>
      <h1 className='header'> Welcome to exclusive app</h1>
        <button onClick={disconnectmetamask} type="button" id='metabtn' className="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2">Quit</button>
        </div>
    )
    }
    ...
    <div id='top'>
      ...
      <Typewriter
       ...
       onInit={(typewrites)=> {
              ...
              typewrites
                .typeString("Join the catto gang")
                .start();
       }}
       />
    </div>
    <div className={styles.container}>
        <h1 className='header'> You are connected as {address}</h1>
        <button onClick={disconnectmetamask} type="button" id='metabtn' className="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2">Disconnect</button>
        
        <button id='mintbtn' onClick={mintNFT} type="button" className="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2">Mint NFT</button>
      </div>
</div>
  )
}
